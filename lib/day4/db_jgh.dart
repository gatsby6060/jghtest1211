...
//   }
  ...
  // 데이터 삭제(함수)
  static Future<int> deleteUser(int userIdd) async {
    final db = await getDatabase();
    return await db.delete('TBL_USER', where: 'userId = ?', whereArgs: [userIdd]);
  }

// 1명 데이터 조회
  static Future<List<Map<String, dynamic>>> getUser(int id) async {
    final db = await getDatabase();
    return await db.query('TBL_USER', where: 'userId = ?', whereArgs: [id]);
  }
